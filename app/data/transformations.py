...
class SalesTransformations:
    ...
    def get_new_lead_data(self, dataframe, engine):
        ...
        df = pd.read_sql('''
                         ...
                         LEFT JOIN public.shopify_customers c 
                         ON s.email_address = c.email_address
                         LEFT JOIN sales_leads.leads_tracking l 
                            ON l.lead_uuid = s.uuid 
                        WHERE c.email is null -- not seen this customer before 
                        AND l.uuid is null    -- and not sent this record previously.
                         ''')
