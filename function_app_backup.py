...
logger.info(os.environ["SENTRY_DSN"])
...
if os.environ.get("FUNCTIONS_ENVIRONMENT") == "preview":
    ...
    logger.info("Running in pre production mode")
    logger.info("Loading pre-prod settings")
    local_settings_path = Path(__file__).parent.joinpath("local.settings.pre_prod.json")
    load_local_settings_as_env_vars(local_settings_path)

# Initialize services with error handling for Azure
try:
...
try:
    if gdrive and psql:
        ...
        logger.info("Sales Transformations service initialized successfully")
    else:
        st = None
        logger.warning("Sales Transformations not initialized due to missing dependencies")
except Exception as e:
    logger.error(f"Failed to initialize Sales Transformations service: {e}")
    st = None
