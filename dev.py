...
if __name__ == '__main__':
    ...
    if not file_dataframe_new.empty:
        ...
        for file in files_to_process.itertuples():
            ...
            az.upload_dataframe(dataframe=dataframe, container_name=f'salesfiles/{parent_name}', blob_name=file.name)
    ...
    #     new_lead_data_zi = st.get_new_zi_search_lead_data()
    ...
    # logging.info('Getting and updating franchise data')
    # franchise_df = gdrive.get_franchise_data(file_id=os.environ.get("FRANCHISE_MASTER_LIST_FILE_ID"))
    ...
    # sheet_url_dict = st.post_city_search_data_to_google_sheet(f'City Search Output - {sheet_week}', os.environ.get("CITY_SEARCH_OUTPUT_PARENT_FOLDER_ID"))
    
    
    # for filename, url in sheet_url_dict.items():
    #     logging.info(f"Posting {filename} to slack")
    #     # psql.post_city_search_slack_message(link=url, spread_sheet_name=filename)
        
    #     for file in files_to_process.itertuples():
    #         logging.info(f"Moving file: {file.name}")
